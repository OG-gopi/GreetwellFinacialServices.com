...
export async function getAllMenus(req: AuthRequest, res: Response) {
  try {
...
export async function createMenu(req: AuthRequest, res: Response) {
  try {
    ...
    // Unique URL Check
    const existingUrl = await prisma.menu.findUnique({ where: { url } });
    if (existingUrl) {
    ...
    const createdWithPerms = await prisma.menu.findUnique({
      where: { id: newMenu.id },
...
export async function toggleMenuStatus(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
...
export async function updateRoleMenuPermissions(req: AuthRequest, res: Response) {
  try {
    ...
    const perm = await prisma.roleMenuPermission.upsert({
...
export async function checkMenuAccess(req: AuthRequest, res: Response) {
  try {
    const { url } = req.query;
